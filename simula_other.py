...
def print_matrix(m):
	...
	for lin in range(length_lattice):
		for col in range(length_lattice):
			if m[lin][col] == 0:
				print(" ",end=''),
			elif m[lin][col] == 1:
				print("*",end='')
...
def cont_prey_predator(m):
	...
	for i in range(length_lattice):
		for j in range(length_lattice):
			if m[i][j] == 0:
				void.append((i,j))
			elif m[i][j] == 1:
				predator.append((i,j))
...
def interation_uniform_resource(matrix,Alpha,Eta,Betha,Lambda,Gamma,f):
	void = []
	prey = []
	predator = []
	length_lattice = len(matrix)
...
lpha= random.random()*Alpha
ta = random.random()*Eta
etha = random.random()*Betha
ambda = random.random()*Lambda
amma = random.random()*Gamma
	while True:
		choice = r.choice([1,2])
...
if __name__ == "__main__":
	...
	Alpha = 0.50          # taxa de crescimento da presa
	Eta = 0.0            # taxa de morte da presa
	Betha= 0.0           # taxa de interação da presa com predador (sem geração de filho)

	Lambda = 0.0         # taxa de morte da predador
	Gamma = 0.0          # taxa de interação do predador com a presa (geração de filho)
		...
		if input("Run? [y/n]") == "y":
			...
			for walking in range(1,Walk+1):
				...
				for step in range(1,Steps+1):
					m = interation_uniform_resource(m, Alpha, Eta, Betha, Lambda, Gamma, neighbor_topology_square_moore)
					print_matrix(m)
					void,predator,prey = cont_prey_predator(m)
